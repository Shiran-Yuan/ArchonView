...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, attn_bias, return_qkv=False):
        B, L, C = x.shape
        ...
        if return_qkv: return self.proj_drop(self.proj(oup)), q, k, v
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, y, pose):
        ...
        using_flash = self.using_flash and q.dtype != torch.float32
        ...
        pose_mat = torch.zeros((B, 1, 1, self.head_dim, self.head_dim), device=k.device)
        for b in range(B):
            for i in range(0, self.head_dim, 4):
                pose_mat[b, 0, 0, i, i] = pose[b, 0]
                pose_mat[b, 0, 0, i, i+1] = -pose[b, 1]
                pose_mat[b, 0, 0, i+1, i] = pose[b, 1]
                pose_mat[b, 0, 0, i+1, i+1] = pose[b, 0]
                pose_mat[b, 0, 0, i+2, i+2] = pose[b, 2]
                pose_mat[b, 0, 0, i+2, i+3] = -pose[b, 3]
                pose_mat[b, 0, 0, i+3, i+2] = pose[b, 3]
                pose_mat[b, 0, 0, i+3, i+3] = pose[b, 2]
        ...
        if using_flash:
            ...
            oup = slow_attn(query=q, key=k, value=v, scale=self.scale, attn_mask=None, dropout_p=dropout_p).transpose(1, 2).reshape(B, L, C)
...
class XAttnBlock(nn.Module):
    def __init__(
        ...
        super(XAttnBlock, self).__init__()
        self.block_idx, self.last_drop_p, self.C = block_idx, last_drop_p, embed_dim
        self.C, self.D = embed_dim, cond_dim
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.attn = CrossAttention(block_idx=block_idx, embed_dim=embed_dim, num_heads=num_heads, attn_drop=attn_drop, proj_drop=drop, attn_l2_norm=attn_l2_norm, flash_if_available=flash_if_available)
        self.ffn = FFN(in_features=embed_dim, hidden_features=round(embed_dim * mlp_ratio), drop=drop, fused_if_available=fused_if_available)
        self.ln_wo_grad = norm_layer(embed_dim, elementwise_affine=False)
    
    def forward(self, x, y, pose):
        x = x + self.drop_path(self.attn(self.ln_wo_grad(x), y, pose))
...
class AdaLNSelfAttn(nn.Module):
    ...
    def forward(self, x, cond_BD, attn_bias):   # C: embed_dim, D: cond_dim
        ...
        att = self.attn( self.ln_wo_grad(x).mul(scale1.add(1)).add_(shift1), attn_bias=attn_bias, return_qkv=False )
        x = x + self.drop_path(att.mul_(gamma1))
