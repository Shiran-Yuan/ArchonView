...
def build_everything(args: arg_util.Args):
    # resume
    auto_resume_info, start_ep, start_it, trainer_state, args_state = auto_resume(args, 'checkpoint.pth')
...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, tb_lg: misc.TensorboardLogger, ld_or_itrt, iters_train: int, trainer):
    ...
    for it, (source, target, pose, emb) in me.log_every(start_it, iters_train, ld_or_itrt, iters_train // 100, header):
        ...
        emb = emb.to(args.device, non_blocking=True)
        ...
        grad_norm, scale_log2 = trainer.train_step(
            ...
            source=source, inp_B3HW=target, pose=pose, emb=emb, prog_si=prog_si, prog_wp_it=args.pgwp * iters_train,
        )
        ...
        if it < 100 and it % 10 == 0: print(it)
