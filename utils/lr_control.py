...
def filter_params(model, nowd_keys=()) -> Tuple[
    ...
    for name, para in model.named_parameters():
        ...
        if name == 'pose_embed.bias':
            cur_wd_sc, group_name = 0., 'PEW'
        elif name == 'pose_embed.weight':
            cur_wd_sc, group_name = 1., 'PEB'
        elif para.ndim == 1 or name.endswith('bias') or any(k in name for k in nowd_keys):
            cur_wd_sc, group_name = 0., 'ND'
        else:
            cur_wd_sc, group_name = 1., 'D'
        if group_name not in para_groups:
            if group_name in ('PEW', 'PEB'): cur_lr_sc = 10.
            else: cur_lr_sc = 1.
